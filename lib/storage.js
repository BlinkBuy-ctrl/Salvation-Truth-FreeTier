...
// Admin-authored feed posts, stored locally until a real backend exists.
export const PostsStore = {
  ...
  async create(post) {
    const current = await this.getAll();
    const next = [{ ...post, id: post.id ?? `post-${Date.now()}`, createdAt: Date.now() }, ...current];
    await safeSet(KEYS.POSTS, next);
    return next;
  },
  async remove(id) {
    const current = await this.getAll();
...
// Per-user reactions (likes) on posts, keyed by userId so guests can't react.
export const ReactionsStore = {
  ...
  async toggleLike(postId, userId) {
    ...
    const likedBy = new Set(all[postId] ?? []);
    ...
    const next = { ...all, [postId]: Array.from(likedBy) };
    await safeSet(KEYS.REACTIONS, next);
    return { likedBy: next[postId], isLiked: !isLiked };
  },
  async getLikeState(postId, userId) {
    const all = await this.getAll();
    const likedBy = all[postId] ?? [];
    return { count: likedBy.length, isLiked: userId ? likedBy.includes(userId) : false };
  },
};
